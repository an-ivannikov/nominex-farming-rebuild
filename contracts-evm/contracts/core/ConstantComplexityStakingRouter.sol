...
contract ConstantComplexityStakingRouter is RecoverableByOwner, INmxSupplier {
    ...
    function changeStakingServiceShares(
        ...
        for (uint256 i = 0; i < shares.length; i++) {
            require(
                ...
                "NmxStakingRouter: zero address is invalid"
            );
    ...
    function supplyNmx(
        ...
        supply = supplyState.share.mulu(
            _totalSupply - supplyState.processedSupply
        );
    ...
    function getRecoverableAmount(
        ...
        for (uint256 i = 0; i < _activeServices.length; i++) {
            ServiceSupplyState storage supplyState = supplyStates[
                _activeServices[i]
            ];
            pendingSupply += supplyState.share.mulu(
                _totalSupply - supplyState.processedSupply
            );
        }
        uint256 balance = IERC20(nmx).balanceOf(address(this));
        require(
            balance >= pendingSupply,
            "NmxStakingRouter: NMX_NEGATIVE_RECOVERABLE_AMOUNT"
        );
    ...
    function pendingSupplies(address service) external view returns (uint256) {
        ...
        return
            supplyState.share.mulu(totalSupply - supplyState.processedSupply);
    }
