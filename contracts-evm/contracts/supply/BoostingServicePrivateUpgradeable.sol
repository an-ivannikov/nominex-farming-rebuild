...
import "../access/RecoverableByOwnerUpgradeable.sol";

contract BoostingServicePrivateUpgradeable is
    UUPSUpgradeable,
    PausableByOwnerUpgradeable,
    RecoverableByOwnerUpgradeable
{
    ...
    function initialize(
        ...
        require(
            ...
            "BoostingService: INVALID_PENALTY_RATE"
        );
        require(
            _performanceFee <= MAX_PERFORMANCE_FEE,
            "BoostingService: INVALID_PERFORMANCE_FEE"
        );
    ...
    function changeParams(
        ...
        uint16 _penaltyRate,
        uint16 _performanceFee
    ) external onlyOwner whenUpgraded {
        _changeParams(
            _nmxSupplier,
            _boostingRate,
            _penaltyRate,
    ...
    function upgradeLaunchPool() external onlyOwner {
        ...
        IERC20(NMX).approve(LAUNCHPOOL, 2 ** 256 - 1);
        IStakingService(LAUNCHPOOL).stakeFrom(
            address(this),
            totalStakedCompounded
        ); // already with compound rewards
    ...
    function _claimReward(
        address owner,
        address spender
    ) private whenUpgraded whenNotPaused {
    ...
    function _unstakeShares(
        ...
        address spender,
        uint128 shares
    ) private whenUpgraded whenNotPaused returns (uint128) {
        Staker storage staker = stakers[owner];
        _compoundAndRecalculateShares(staker, 0, owner);
    ...
    function _unstakeSharesLocked(
        ...
        uint128 principalDiff = uint128(
            (uint256(staker.principalAmount) * shares) / _stakerShares
        );
        uint128 amountDiff = uint128(
            (uint256(staker.amount) * shares) / _stakerShares
        );
        uint128 boostingDiff = uint128(
            (uint256(staker.boostingAmount) * shares) / _stakerShares
        );
        ...
        uint128 penaltyAmount = (principalDiff * penaltyRate) /
            RATE_DENOMINATOR;
    ...
    function _unstakeSharesUnlocked(
        Staker storage staker,
        address owner,
        address spender,
        uint128 shares
    ...
    function _compoundAndRecalculateShares(
        ...
        uint128 amount,
        address owner
    ) private returns (uint128 adjustedStakedAmount) {
        uint128 compoundRewards = _receiveCompound();
        _receiveBoosting(compoundRewards);
    ...
    function _receiveCompound() private returns (uint128 claimedReward) {
        ...
        if (upgraded) {
            ...
            claimedReward = uint128(
                IStakingService(OLD_LAUNCHPOOL).claimReward()
            );
        }
        if (_fullShares > 0) {
    ...
    function _receiveBoosting(uint128 compoundRewards) private {
        ...
        uint128 _totalShares = totalShares;
        if (_totalShares > 0) {
            uint128 _historicalBoostingRate = historicalBoostingRate +
                ((boostingRewards << 40) / _totalShares);
    ...
    function _materializeShares(
        Staker storage staker,
        address owner
    ) private returns (uint128 principalAmountDiff) {
        uint128 _stakerShares = staker.shares;
        uint128 _stakerInitialBoostingRate = staker.initialBoostingRate;

        uint128 compoundAmount = uint128(
            ((uint168(historicalCompoundRate) - staker.initialCompoundRate) *
                _stakerShares) >> 40
        );
        uint128 boostingAmount = uint128(
            ((uint168(historicalBoostingRate) - _stakerInitialBoostingRate) *
                _stakerShares) >> 40
        );
        ...
        if (stakingEnd < block.timestamp) {
            ...
            if (_stakedAt > 0) {
                ...
                uint128 boostingRateAtCheckpoint = _findNearestLowestBoostingRate(
                        stakingEnd
                    );
                uint128 boostingAmountAtCheckpoint = uint128(
                    ((uint168(boostingRateAtCheckpoint) -
                        _stakerInitialBoostingRate) * _stakerShares) >> 40
                );
    ...
    function _stakeShares(Staker storage staker, uint128 amount) private {
        ...
        uint128 _totalShares = totalShares;
        if (_totalShares != 0) {
            currentShares = uint128(
                (uint256(amount) * _totalShares) / totalStakedCompounded
            );
        } else {
            require(
                amount >= 10 ** 16,
                "BoostingService: INVALID_INITIAL_AMOUNT"
            );
        ...
        uint64 time = _recalculateStakingTime(
            staker.stakedAt,
            prevPrincipalAmount,
            amount
        );
    ...
    function _refundSupplier(uint128 amount) private {
        bool transferred = IERC20(NMX).transfer(
            REFUND_SUPPLIER_ADDRESS,
            amount
        );
    ...
    function _findNearestLowestBoostingRate(
        uint64 timestamp
        ...
        while (high > low) {
            ...
            BoostingRateCheckpoint storage checkpoint = boostingRateHistory[
                mid
            ];
