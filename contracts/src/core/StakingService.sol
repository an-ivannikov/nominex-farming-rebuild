...
contract StakingService is PausableByOwner, RecoverableByOwner {
    ...
    function _unstake(address from, address to, uint128 amount) private {
        ...
        require(
            staker.amount >= amount,
            "NmxStakingService: NOT_ENOUGH_STAKED"
        );
    ...
    function claimWithAuthorization(
        ...
        require(
            nmxAmount <= signedAmount,
            "NmxStakingService: INVALID_NMX_AMOUNT"
        );
        verifySignature(
            CLAIM_TYPEHASH,
            owner,
    ...
    function _claimReward(
        ...
        require(
            ...
            "NmxStakingService: NOT_ENOUGH_BALANCE"
        );
    ...
    function updateHistoricalRewardRate() public {
        ...
        if (state.totalStaked != 0) {
            uint128 additionalRewardRate = uint128(
                (currentNmxSupply << 40) / state.totalStaked
            );
    ...
    function getRecoverableAmount(
        ...
        require(
            tokenAddress != nmx,
            "NmxStakingService: INVALID_RECOVERABLE_TOKEN"
        );
